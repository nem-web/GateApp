...
interface PageProps {
  params: Promise<{
    slug: string; // e.g., "gate-ee", "gate-cs"
  }>;
}

export default async function PyqBranchPage({ params }: PageProps) {
  ...
  // 2. Fetch the pyq.json data for this specific branch
  ...
  return (
    <div className={`min-h-screen bg-[#0e0f14] text-white font-sans pb-24 ${config.theme.selection}`}>
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-4 pt-12 sm:px-6 lg:px-8">
        ...
        {/* Back Navigation to the main PYQ index */}
        <Link 
          href="/pyq" 
        ...
        {/* Reuse the premium UI component! 
          We pass "pyq" as the resourceType so it automatically picks the FileText icon.
        */}
        <ResourceList 
          data={data} 
          config={config} 
