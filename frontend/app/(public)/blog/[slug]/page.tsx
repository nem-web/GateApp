...
const reactions = [
  { emoji: "👍", value: "LIKE" },
  { emoji: "⬆️", value: "UPVOTE" },
  { emoji: "⬇️", value: "DOWNVOTE" },
  { emoji: "❤️", value: "LOVE" },
];
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  const totalReactions = reactionCounts.reduce(
    (sum, reaction) => sum + reaction._count,
    0
  );
  ...
  async function postComment(formData: FormData) {
    ...
    if (existingComments >= 5) {
      throw new Error(
        "Maximum 5 comments allowed per blog"
      );
    }
  ...
  const reactionValue =
    formData.get("reaction") as ReactionValue;
  ...
  if (existing) {
    if (existing.value === reactionValue) {
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-12 px-4 py-10 md:py-16">
        ...
        <section className="space-y-8" id="comments">
          <h2 className="text-2xl font-bold tracking-tight">Discussion ({totalComments})</h2>
