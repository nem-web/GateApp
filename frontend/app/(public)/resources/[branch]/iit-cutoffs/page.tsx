...
export default async function DynamicIitCutoffsPage({ params }: PageProps) {
  ...
  // Fetch the JSON data
  const dataFilePath = path.join(process.cwd(), "data", branch, "iit-cutoffs.json");
  let cutoffData: CutoffData;
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-[#15161b]/80 border-b border-gray-800 text-gray-400 backdrop-blur-sm">
                <tr>
                  ...
                  <th className="p-6 font-semibold text-center tracking-wider text-gray-300">OBC-NCL</th>
                  <th className="p-6 font-semibold text-center tracking-wider text-gray-500">SC/ST</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-800/50">
                {cutoffData.cutoffs.map((row, i) => (
                  ...
                  >
                    ...
                    <td className="p-6 text-center text-gray-400 group-hover:text-gray-300">{row.obc}</td>
                    <td className="p-6 text-center text-gray-500 group-hover:text-gray-400">{row.scst}</td>
                  </tr>
                ))}
              </tbody>
            </table>
