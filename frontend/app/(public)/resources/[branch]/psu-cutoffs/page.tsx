...
interface PsuCutoffData {
  pageSubtitle: string;
  cutoffs: PsuCutoffRow[];
}

export default async function DynamicPsuCutoffsPage({ params }: PageProps) {
  ...
  // Fetch the JSON data
  const dataFilePath = path.join(process.cwd(), "data", branch, "psu-cutoffs.json");
  let cutoffData: PsuCutoffData;
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-[#15161b]/80 border-b border-gray-800 text-gray-400 backdrop-blur-sm">
                <tr>
                  <th className="p-6 font-semibold tracking-wider">PSU Name</th>
                  <th className="p-6 font-semibold tracking-wider">Role Profile</th>
                  <th className={`p-6 font-bold text-center tracking-wider ${branchColorClass}`}>General</th>
                  <th className="p-6 font-semibold text-center tracking-wider text-blue-400">OBC-NCL</th>
                  <th className="p-6 font-semibold text-center tracking-wider text-purple-400">SC</th>
                  <th className="p-6 font-semibold text-center tracking-wider text-pink-400">ST</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-800/50">
                {cutoffData.cutoffs.map((row, i) => (
                  ...
                  >
                    ...
                    <td className="p-6 font-bold text-white group-hover:text-gray-100">{row.name}</td>
                    <td className="p-6 text-gray-400 group-hover:text-gray-300">{row.role}</td>
                    
                    {/* Highlighted General Score */}
                    ...
                    <td className="p-6 text-center text-gray-300 group-hover:text-white transition-colors">{row.obc.toFixed(2)}</td>
                    <td className="p-6 text-center text-gray-400 group-hover:text-gray-300 transition-colors">{row.sc.toFixed(2)}</td>
                    <td className="p-6 text-center text-gray-500 group-hover:text-gray-400 transition-colors">{row.st.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
