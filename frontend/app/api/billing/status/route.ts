...
import { getUserPlan, PLAN_FEATURE_MATRIX, PLAN_LIMITS, countFeatureUsage, formatCurrencyPaise, PREMIUM_AMOUNT_PAISE } from "@/lib/subscription";
...
export async function GET() {
  ...
  const usagePairs = await Promise.all(
    (Object.keys(PLAN_LIMITS.TRIAL) as Array<keyof typeof PLAN_LIMITS.TRIAL>).map(async (feature) => [
      feature,
      feature === "storage_bytes" ? memory.usedBytes : await countFeatureUsage(userId, feature),
    ]),
  );
  ...
  return NextResponse.json({
    plan: {
      ...
      activeSubscription: plan.activeSubscription ?? null,
      limits: plan.limits,
    },
    pricing: {
    ...
    usage: Object.fromEntries(usagePairs),
    memory,
